          ...
          >
            <div className="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] gap-4 w-full">
              {category.skills.map((skill) => (
                <div
                  key={skill.name}
                  className="flex items-center justify-center gap-2 bg-transparent border border-gray-700 rounded-full py-2.5 px-6 transition-all duration-300 hover:border-[#8245ec]"
                >
                  <img
                    src={skill.logo}
                    alt={`${skill.name} logo`}
